      ...
      if (
        ...
        switch (node.property.name) {
          ...
          case "host":
            ...
            break;
          case "searchParams":
            replacementsStage1.push({
              start: node.start,
              end: node.end,
              value: `new URLSearchParams(${JSON.stringify(targetUrlCalc.search)})`
            });
            break;
          case "origin":
  ...
  walk.ancestor(ast, {
    AssignmentExpression(node, ancestors) {
      if (
        ...
        const parent = ancestors[ancestors.length - 2];
        const isExprStmt = parent && parent.type === "ExpressionStatement";

        const call = `window.parent.loadPage(new URL(${rhsText}, ${JSON.stringify(targetUrl)}).href)${isExprStmt ? ";" : ""}`;
        replacementsStage2.push({ start: node.start, end: parent?.end ?? node.end, value: call });
      }
    },
    ...
    CallExpression(node, ancestors) {
      ...
      if (
        ...
        const call = `window.parent.loadPage(new URL(${argCode}, ${JSON.stringify(targetUrl)}).href)${isExprStmt ? ";" : ""}`;
        replacementsStage2.push({ start: node.start, end: parent?.end ?? node.end, value: call });
      }
    }
  });
